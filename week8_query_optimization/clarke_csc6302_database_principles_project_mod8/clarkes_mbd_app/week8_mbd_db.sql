...
CREATE VIEW caloriesPerDaySummaries AS
    ...
    LEFT JOIN users u ON u.user_id = m.user_id
    GROUP BY
		m.user_id, m.meal_id, date, u.username, f.food_name, mi.servings;
...
/*-----------This procedure validates a user-----------*/
CREATE PROCEDURE validateUser(
    uusername VARCHAR(100), 
    upassword_hash VARCHAR(255)
)
...
BEGIN
    ...
    -- Check if username exists (your original approach)
    SELECT getUserName(uusername) INTO foundUserName;

    SELECT password_hash 
      INTO foundUserpassword
      FROM users
	...
	ELSEIF foundUserpassword IS NULL OR foundUserpassword <> upassword_hash
	THEN 
        SELECT -2 AS passwordNotFound;

    ELSE
		-- return the matching account row
		SELECT username, password_hash 
          FROM users 
         WHERE username = uusername 
           AND password_hash = upassword_hash
         LIMIT 1;
...
BEGIN
    ...
    ELSE
        ...
        -- return the user ID of the user that was just created
        SET result_id = LAST_INSERT_ID();
	END IF;
    
    -- return a SINGLE result set with either -1, -2, or the new user_id
    SELECT result_id AS userID;
...
BEGIN
    ...
    ELSE
        ...
        SELECT LAST_INSERT_ID() AS MealCreatedID;
...
BEGIN
    ...
    -- Look up email for the resolved user_id
    SELECT email
		FROM users
    ...
    -- Username not found
    IF foundUserID IS NULL OR foundUserID = -1
    THEN
        SELECT -1 AS userNotFound;

    -- Email not found
    ELSEIF foundEmail IS NULL OR foundEmail = -1
    THEN
        SELECT -2 AS emailNotFound;

    -- Username exists but email does not match
    ELSEIF emailLookUp IS NULL OR emailLookUp != foundEmail
    THEN
        SELECT -3 AS emailNotMatched;

    -- Valid username + email → update password
    ELSE
		UPDATE users
        SET password_hash = myNewPassowrd
        WHERE user_id = foundUserID
          AND email = foundEmail;
